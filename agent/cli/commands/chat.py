...
def handle_slash_command(user_input: str, settings, convo: Conversation, client: LLMClient):
    """Handle slash commands (/help, /model, etc.)."""
    ...
    if command in {"help", "h", "?"}:
        ...
        system_prompt = next((msg["content"] for msg in convo.history() if msg.get("role") == "system"), "")
        ...
        current_system = next((msg["content"] for msg in convo.history() if msg.get("role") == "system"), "")
...
def _handle_help_command(args: list[str]):
    print("\nSlash Commands:")
...
def _handle_model_command(args: list[str], settings, client: LLMClient):
    import requests
    ...
    if not args:
        ...
        try:
            ...
            if resp.status_code != 200:
                print(f"Error: Could not fetch models (HTTP {resp.status_code})")
                return
            data = resp.json()
    ...
    if target.isdigit():
        try:
            ...
            idx = int(target) - 1
            if 0 <= idx < len(models):
                target = models[idx].get("name", target)
            else:
...
def _handle_config_command(args: list[str], settings):
    masked_key = "*" * len(settings.api_key) if settings.api_key else "(not set)"
...
def _handle_tools_command(args: list[str]):
    categories = {
        "File Operations": ["create_file", "read_file", "write_file", "edit_file", "copy_path", "rename_path", "delete_path", "rename_all", "rename_semantic", "list_dir", "list_tree", "glob_files"],
        "Search": ["search_text", "search_index", "code_search", "web_search", "find_symbol"],
        "Code Execution": ["run_python", "python_exec", "run_shell", "run_powershell", "run_ssh", "run_background"],
    ...
    for category, tools in categories.items():
        ...
        if available:
            print(f"{category}:")
            for t in available:
                tool_obj = next((x for x in TOOLS if x["function"]["name"] == t), None)
                desc = tool_obj["function"].get("description", "") if tool_obj else ""
                print(f"  • {t:<25} {desc[:60]}")
            print()
...
def _handle_history_command(args: list[str], convo: Conversation):
    ...
    for idx, msg in enumerate(history[-limit:], 1):
        ...
        if len(content) > 200:
            content = content[:197] + "..."
        role_display = {"system": "SYS", "user": "YOU", "assistant": "AI ", "tool": "TOOL"}.get(role, role.upper())
        print(f"{idx}. [{role_display}] {content}")
    print(f"\nTotal messages: {len(history)}")


def _handle_clear_command(args: list[str], convo: Conversation, system_prompt: str):
...
def _handle_save_command(args: list[str], convo: Conversation, settings):
    if args:
        save_path = Path(" ".join(args))
    else:
        ts = datetime.now().strftime("%Y%m%d-%H%M%S")
        save_path = settings.data_dir / "sessions" / f"manual-save-{ts}.jsonl"
...
def _handle_load_command(args: list[str], convo: Conversation, settings):
    if not args:
    ...
    try:
        ...
        for line in load_path.read_text(encoding="utf-8").splitlines():
            ...
            loaded += 1
        print(f"✓ Loaded {loaded} messages from: {load_path}")
    except Exception as exc:
...
def _handle_transcripts_command(args: list[str], settings):
    transcript_dir = Path(settings.data_dir) / "sessions"
    if not transcript_dir.exists():
        print(f"\nNo transcripts directory at: {transcript_dir}")
        return
    ...
    for idx, tpath in enumerate(transcripts[:20], 1):
        size = tpath.stat().st_size // 1024
        modified = datetime.fromtimestamp(tpath.stat().st_mtime).strftime("%Y-%m-%d %H:%M")
        try:
            line_count = sum(1 for _ in tpath.open())
        except Exception:
            line_count = "?"
        print(f"{idx}. {tpath.name:<40} ({line_count} msgs, {size}KB, {modified})")
